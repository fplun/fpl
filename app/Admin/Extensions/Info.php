...
class Info extends AbstractDisplayer
{
...
$('.grid-info').on('click', function () {
    if ($(this).data('inserted') == '0') {
        var key = $(this).data('key');
        var row = $(this).closest('tr');
        var html = $('template.grid-info-'+key).html();
...
EOT;
        ...
        return <<<EOT
<a class="btn btn-xs btn-default grid-info" data-inserted="0" data-key="{$key}" data-toggle="collapse" data-target="#grid-collapse-{$key}">
    <i class="fa fa-caret-right"></i> $btn
</a>
<template class="grid-info-{$key}">
