			...
			if (!options.disableSwipe) {
				$viewer.swipe( {
					swipe:function(event, direction, distance, duration, fingerCount, fingerData) {
						...
						if (direction == "right" && options.disableSwipe_direction=='cross') goto(pageNum-1);
						else if (direction == "left" && options.disableSwipe_direction=='cross') goto(pageNum+1);
						else if (direction == "down" && options.disableSwipe_direction=='leng') goto(pageNum-1);
						else if (direction == "up" && options.disableSwipe_direction=='leng') goto(pageNum+1);
					},
					threshold:50,
		...
		*/
		function renderPage () {
			...
			pdfDoc.getPage(pageNumRendering).then(function(page) {		
				...
				var bl=[];
				if(options.canvas_width && !isNaN( options.canvas_width) &&  Number(options.canvas_width)>0){
					bl[0]= options.canvas_width/viewport.width;
				}
				if(options.canvas_height && !isNaN(options.canvas_height) &&  Number(options.canvas_height)>0){
					bl[1]= options.canvas_height/viewport.height;
				}
				if(bl.length>0){
					if(bl[0]>bl[1]){
						viewport = page.getViewport(options.pdfScale*options.quality*bl[1]);
					}else{
						viewport = page.getViewport(options.pdfScale*options.quality*bl[0]);
					}
				}
		...
		function redraw() {
			...
			if (state == INIT) {
				...
				if(options.height && !isNaN(options.height) && Number(options.height)>0){
					var pdfHeight = options.height / options.quality;
				}else{
					var pdfHeight = canvas.height / options.quality;
				}
				if(options.width && !isNaN(options.width) && Number(options.width)>0){
					var pdfWidth = options.width / options.quality;
				}else{
